...
class EmailTemplatesHandler(PersistentServerConnectionApplication):
    ...
    def _delete_email_template(self, sessionKey, user, post_data):
        ...
        query = {}
        query['_key'] = key
        logger.debug("Query for email templates: {}".format(urllib.parse.quote(json.dumps(query))))
        ...
        return self.response('Email Template with key {} successfully removed'.format(key), http.client.OK)
    ...
    def _update_email_templates(self, sessionKey, user, post_data):
        ...
        return self.response('Email Templates successfully updated', http.client.OK)
