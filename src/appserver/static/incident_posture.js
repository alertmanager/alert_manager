...
require([
    ...
    // [START] new version
    // let confSettings = false;
    const getConfig = async () => {
        // console.log( "getConfig() [started]" )
        const settings = await fetch('/splunkd/services/configs/conf-alert_manager/settings?output_mode=json')
        .then( res => {
            ...
            return ( fetchedSettings ) ? fetchedSettings.entry[0].content : false;
        }).catch( err => {
            console.error( "[FAIL]: could not fetch 'incident_list_length'", err )
        } )
        if ( settings ){
            defaultTokens.set('incident_list_length', settings.incident_list_length);
            submittedTokens.set('incident_list_length', settings.incident_list_length);
        }
        // confSettings = settings;
        initIncidentTable( settings )
        // console.log( "getConfig() [executed]", confSettings )
    }
    getConfig()
    // [END]
    ...
    // [START] incidentsOverViewTable
    const initIncidentTable = ( confSettings ) => {
        ...
        incidentsOverViewTable.on( "rendered", function ( obj ) {
            ...
            if ( !rendered ) {
                ...
                // console.log( 'confSettings', confSettings )
                if( confSettings ) {
                    obj.settings.set( { pageSize: confSettings.incident_list_length } );
                }
