...
'</div>';
            ...
            var url = splunkUtil.make_url('/custom/alert_manager/email_settings/save_templates');
    ...
    $(document).on("click", "#save_settings", function(event){
        ...
        console.debug("save settings data", data);
        ...
        // validate data
        var check = _.filter(data, function(entry){ 
            return entry['alert']== null || entry['notify_user_template'] == null; 
        });
        console.debug("check", check);
        if (check.length>0) {
            var modal = ''+
...
'</div>';
            ...
            var url = splunkUtil.make_url('/custom/alert_manager/email_settings/save_settings');
            console.debug("url", url);
